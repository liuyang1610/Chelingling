...
public class MyAddAddRessActivity
    ...
    private Handler handler = new Handler(new Handler.Callback() {
        public boolean handleMessage(Message paramAnonymousMessage) {
            if (paramAnonymousMessage.arg1 == 0) {
                Toast.makeText(MyAddAddRessActivity.this, "保存成功", Toast.LENGTH_SHORT).show();
                MyAddAddRessActivity.this.finish();
                return false;
            }
            Toast.makeText(MyAddAddRessActivity.this, "保存失败 请重试", Toast.LENGTH_SHORT).show();
            return false;
        }
    });
    ...
    private void postByOkGo(String paramString) {
        ...
        ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) ((PostRequest) OkGo.post(paramString).tag(this)).cacheKey("cachePostKey")).cacheMode(CacheMode.DEFAULT)).params("appid", this.strs[0], new boolean[0])).params("appsecret", this.strs[1], new boolean[0])).params("name", this.shouhuorenna.getText().toString().trim(), new boolean[0])).params("tel", this.dianphone.getText().toString().trim(), new boolean[0])).params("add", this.diqushengshi.getText().toString().trim(), new boolean[0])).params("adds", this.xiangxiadd.getText().toString().trim(), new boolean[0])).params("zt", this.a, new boolean[0])).params("timestamp", Util.getTime(), new boolean[0])).params("sign", Util.computeSha1OfString("appid=" + this.strs[0] + "&appsecret=" + this.strs[1] + "&timestamp=" + Util.getTime()), new boolean[0])).execute(new StringCallback() {
            ...
            public void onSuccess(String paramAnonymousString, Call paramAnonymousCall, Response paramAnonymousResponse) {
                ...
                try {
                    ...
                    Message message = new Message();
                    message.arg1 = 1;
                    MyAddAddRessActivity.this.handler.sendMessage(message);
                    return;
    ...
    protected void onCreate(Bundle paramBundle) {
        ...
        setContentView(R.layout.activity_my_add_add_ress);
        ButterKnife.bind(this);
        getWindow().clearFlags(1024);
        this.titles.setText("添加地址");
        this.strs = Util.load(this).split(",");

        this.switchView.setOnStateChangedListener(new SwitchView.OnStateChangedListener() {
...
/* Location:              G:\chelingling\dex2jar-2.0\classes-dex2jar.jar!\com\beijing\chelingling\MyAddAddRessActivity.class
